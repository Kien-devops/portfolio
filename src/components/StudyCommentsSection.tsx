...
import { Comment, fetchStudyComments, postStudyComment, postStudyReply } from '@/utils/api';
import { MessageSquare, Reply, Send, Loader2, AlertCircle } from 'lucide-react';

interface StudyCommentsSectionProps {
  studyId: string;
}
...
export default function StudyCommentsSection({ studyId }: StudyCommentsSectionProps) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        setLoading(true);
        const data = await fetchStudyComments(studyId);
  ...
  const handleCommentSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const newComment = await postStudyComment(studyId, {
  ...
  const handleReplySubmit = async (e: React.FormEvent, parentId: string) => {
    ...
    try {
      ...
      const newReply = await postStudyReply(studyId, parentId, {
